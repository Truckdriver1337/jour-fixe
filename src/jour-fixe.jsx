...
function VideoPlayer({ video }) {
  ...
  const isVertical = ['youtube-shorts', 'instagram'].includes(video.platformKey);

  if (isVertical) {
    return (
      <div style={{ background: '#000', display: 'flex', justifyContent: 'center', padding: '16px 0', position: 'relative' }}>
        <div style={{ width: 'min(380px, 90%)', aspectRatio: '9/16', maxHeight: '72vh' }}>
          <iframe
            key={video.id}
            src={video.embedUrl}
          ...
          {current ? (
            <>
              <div style={{ position: 'relative' }}>
                ...
                <EmoteOverlay emotes={session.emotes || []} />
              </div>
...
function GuestView({ code, userId, userName, onLeave }) {
  ...
  const addVideo = async () => {
    ...
    if (!parsed) {
      setAddError("doesn't look like a YouTube or Instagram link.");
      return;
    }
    setAdding(true);
